...
setwd("/Users/gracesong/Library/Mobile Documents/com~apple~CloudDocs/Documents/Emory/QTM 302W/EDA_project/data")

# Load the 2015-2019 dataset
data_2015 <- read.csv("IHME_USA_DIABETES_COUNTY_RACE_ETHN_2000_2019_MX_2015_BOTH_Y2025M02D10.csv")
data_2016 <- read.csv("IHME_USA_DIABETES_COUNTY_RACE_ETHN_2000_2019_MX_2016_BOTH_Y2025M02D10.csv")
data_2017 <- read.csv("IHME_USA_DIABETES_COUNTY_RACE_ETHN_2000_2019_MX_2017_BOTH_Y2025M02D10.csv")
data_2018 <- read.csv("IHME_USA_DIABETES_COUNTY_RACE_ETHN_2000_2019_MX_2018_BOTH_Y2025M02D10.csv")
data_2019 <- read.csv("IHME_USA_DIABETES_COUNTY_RACE_ETHN_2000_2019_MX_2019_BOTH_Y2025M02D10.csv")
```
...
ggplot(aggregated_data, aes(x = race_name, y = annual_death_rate, fill = race_name)) +
  ...
  labs(title = "Annual Death Rate by Race (Sorted)",
       x = "Race",
       y = "Annual Death Rate") +
  theme_minimal() +
...
hist(AIAN_data$value_scaled, 
     main = "Figure 1: Distribution of Diabetes Mortality Rates", 
...
Figure 1 is a bar plot displaying the distribution of diabetes-related mortality rates among non-Hispanic American Indian or Alaska Native (AIAN) individuals from 2015 to 2019.
...
We want to investigate whether such trend holds true for AIAN group, and if age can account for the large variability observed from figure 1.
...
boxplot(value_scaled ~ age_name, data = AIAN_data, 
        las = 2,                               
        main = "Figure 2: Diabetes Mortality across age group", 
        xlab = "",                            
...
Figure 2 is a boxplot that visually represents diabetes mortality rates across different age groups within the AIAN population.
...
ggplot(age_distribution, aes(x = age_group, y = count)) +
  ...
  labs(title = "Figure 3: Distribution of Diabetes Mortality Across Age Groups",
       x = "", 
...
Figure 3 is a bar chart displaying the distribution of diabetes-related mortality cases across different age groups.
...
ggplot(age_trend, aes(x = year, y = mean_death_rate, color = age_group)) +
  ...
  geom_point(size = 1) +
  labs(
    title = "Figure 4: Diabetes Death Rate Trends by Age Group",
    x = "Year",
...
ggplot(age_summary, aes(x = reorder(age_group, mean_unit_death), y = mean_unit_death, fill = age_group)) +  
  ...
  labs(title = "Figure 5: Diabetes Mortality Across Age Groups",
       x = "", 
...
Figure 5 is a bar chart that represents the average diabetes-related death rate per 10,000,000 people across different age groups.
...
However, while the total number of deaths in these groups (Figure 3) is nonzero, their mortality rate remains very low when adjusted for population size.
While Figure 3 showed the absolute number of deaths, this figure adjusts for population size, providing a clearer picture of which age groups face the highest risk.
...
ggplot(county_summary, aes(x = reorder(location_name, avg_unit_death), y = avg_unit_death)) +
  ...
  coord_flip() +
  labs(title = "Figure 6: Top 10 counties with highest diabetes mortality",
...
Figure 5 is a flipped bar plot visualization of the diabetes mortality across counties, which is displayed using a horizontal format (coord_flip) for better readability.
...
ggplot(location_avg, aes(x = reorder(location_name, avg_death_unit), y = avg_death_unit)) +
  geom_bar(stat = "identity",fill = "steelblue") +
  coord_flip() +
  labs(title = "Figure 7: Top 10 states with highest AIAN diabetes mortality",
       x = "Location", y = "Average Death per 10,000,000") +
  theme_minimal()+
   theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 12)
...
Figure 6 is a flipped bar plot visualization of the diabetes mortality across states.
Applying the same format as Figure 5, the states are reordered based on their mortality rates to ensure that the state with the highest mortality appears at the top.
The resulting Figure 6 reveals significant geographic disparities in diabetes-related mortality among AIAN populations, with Maine and Rhode Island as the top two states with the highest mortality rate.
...
ggplot(map_data, aes(long, lat, group = group, fill = avg_death_unit)) +
  ...
  theme_void() +  
  labs(
    title = "Figure 8: AIAN Diabetes Death per 10,000,000 Across States"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
...
In Figure 7, each state is filled with a color corresponding to its mortality rate.
...
From Figure 7, it becomes evident that mortality rates among the AIAN population are higher in the Northern and central regions of the U.S.
